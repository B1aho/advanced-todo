...
export function deserialize(json, proto) {
  ...
  // Serialize sets
  if (obj.subtask) obj.subtask = new Set(obj.subtask);
  if (obj.todos) obj.todos = new Set(obj.todos);
  if (obj.sections) obj.sections = new Set(obj.sections);
  ...
  serializeInfo.meta.getters.forEach((key) => {
    ...
    if (descriptor.get) Object.defineProperty(obj, key, descriptor);
  });

  serializeInfo.meta.setters.forEach((key) => {
    const descriptor = Object.getOwnPropertyDescriptor(proto, key);
    if (descriptor.set) Object.defineProperty(obj, key, descriptor);
  });
